...
void DSVisualizerTab::drawNode(QGraphicsScene* scene, double x, double y, double r,
    ...
    QGraphicsTextItem* labelText = scene->addText(label, QFont("Segoe UI", 7, QFont::Bold));
    ...
    if (!sub.isEmpty()) {
        QGraphicsTextItem* subText = scene->addText(sub, QFont("Segoe UI", 6));
...
void DSVisualizerTab::refresh() {
    ...
    if (m_analyzer->hasLastImpact) {
        ...
        ).arg(QString::fromStdString(ProcessCollector::cleanName(imp.processName)))
...
void DSVisualizerTab::drawHeap() {
    ...
    for (int i = 0; i < (int)top.size(); i++) {
        ...
        QString name = QString::fromStdString(
            ProcessCollector::cleanName(top[i].name)).left(10);
...
void DSVisualizerTab::drawRBTree() {
    ...
    for (auto& np : positions) {
        ...
        QString name = QString::fromStdString(ProcessCollector::cleanName(p.name)).left(10);
    ...
    for (int level = maxLevel; level >= 0; level--) {
        ...
        for (int i = 0; i < showCount; i++) {
            ...
            auto* text = m_skipScene->addText(
                QString::fromStdString(ProcessCollector::cleanName(sorted[i].name)).left(8),
...
void DSVisualizerTab::drawLRU() {
    ...
    for (int i = 0; i < showCount; i++) {
        ...
        auto* nameText = m_lruScene->addText(
            QString::fromStdString(ProcessCollector::cleanName(recency[i].name)).left(10),
            QFont("Segoe UI", 7, QFont::Bold));
