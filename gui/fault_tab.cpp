...
void FaultTab::setupUI() {
    ...
    // Correlation Card
    m_corrCard = new QFrame();
    m_corrCard->setStyleSheet(
        "QFrame { background: #FFFFFF; border: 1px solid #E2E8F0;"
        "  border-radius: 8px; }"
    );
    QVBoxLayout* corrLayout = new QVBoxLayout(m_corrCard);
...
void FaultTab::refresh() {
    ...
    for (int row = 0; row < (int)faulters.size(); row++) {
        ...
        m_topFaultersTable->setItem(row, 0, makeItem(
            QString::fromStdString(Analyzer::cleanName(p.name))));
